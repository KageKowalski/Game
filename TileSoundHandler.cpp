...
bool TileSoundHandler::attachTileSounds(sf::Time deltaTime, sf::FloatRect cameraView, const sf::Vector2f& pposition, const TileMap& tilemap )
{
    for(int i = 0; i < tilemap.getWidth(); i++)
    {
        ...
        {
            if(cameraView.contains(tilemap.getGroundTiles()[i][j].getCenterPosition().x, tilemap.getGroundTiles()[i][j].getCenterPosition().y))
            {
                if(tilemap.getGroundTiles()[i][j].getSoundFilename().length())
                {
                    ...
                    else
                    {
                        ...
                        float radius = sqrt(pow(abs(pposition.x-tilemap.getGroundTiles()[i][j].getPosition().x),2)+pow(abs(pposition.y-tilemap.getGroundTiles()[i][j].getPosition().y),2));
...
void TileSoundHandler::playSounds(sf::Time deltaTime)
{
    for(int i = 0; i < 32; i++)
    {
